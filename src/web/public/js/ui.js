...
export function setupUIListeners() {
    ...
    muteRemoteBtn.addEventListener('click', () => {
        const audioEl = document.getElementById('user-2-audio');
        audioEl.muted = !audioEl.muted;
        muteRemoteBtn.querySelector('svg path').setAttribute('d', audioEl.muted ? SPEAKER_OFF : SPEAKER_ON);
        muteRemoteBtn.title = audioEl.muted ? 'Unmute' : 'Mute';
        muteRemoteBtn.setAttribute('aria-label', audioEl.muted ? 'Unmute remote audio' : 'Mute remote audio');
    });
